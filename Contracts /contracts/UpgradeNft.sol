...
contract UpgradeNft is Initializable, VRFConsumerBaseV2Upgradable, ERC1155Upgradeable, OwnableUpgradeable, ERC1155URIStorageUpgradeable {
    // Dex Address
    ...
    mapping(uint256 => UpgradeSpecifications) public tokenIdToUpgradeSpecifications; 
    ...
    // set Dex address 
    function setDex(address _dexAddress) public onlyOwner {
        dexAddress = _dexAddress;
    }

    // set BedroomNft address
    function setBedroomNft(BedroomNftInterface _bedroomNftAddress) public onlyOwner {
        bedroomNftInstance = _bedroomNftAddress;
    }

    // Set Upgrade Value Max
    function setUpgradeValueMax(
        uint256 _newValue
    ) public onlyOwner {
    ...
    function mintingUpgradeNft(uint256 _newDesignId, uint256 _upgradeDesignId, uint256 _price, uint256 _indexAttribute, address _owner) external {
        ...
        tokenIdToUpgradeSpecifications[tokenId] = UpgradeSpecifications(_indexAttribute, 0, _owner, _price, _newDesignId,  _upgradeDesignId);
    ...
    // Get Token Name
    function getName(uint256 _tokenId) public pure returns (string memory) {
        return string(
            abi.encodePacked(
                "Token #",
    ...
    function _mintingUpgradeNft(uint256 _tokenId, uint256[] memory _randomWords) internal {
        // Create new random upgrade 
        tokenIdToUpgradeSpecifications[_tokenId].attributeValue = (_randomWords[0] % upgradeValueMax) + 1;
        ...
        string memory DesignName = string(
            abi.encodePacked(
                Strings.toString(tokenIdToUpgradeSpecifications[_tokenId].upgradeDesignId), 
                fileFormat
            )
        );
        ...
        require(address(bedroomNftInstance) != address(0), "BedroomNftInstance not initialized");
